...
// Helper function to create default business data
async function createDefaultBusinessData(businessId: string, validatedData: { name: string; email: string; phone?: string; city: string; businessAddress: string }) {
  ...
  // יצירת שירות ברירת מחדל "שירות כללי"
  const service = await prisma.service.create({
    data: {
      businessId: businessId,
      name: 'שירות כללי',
      durationMin: 30, // 30 דקות
  ...
  // יצירת לקוח לדוגמא - ישראל ישראלי
  ...
  // יצירת תור לדוגמא - לעוד שעה מרגע ההרשמה, לשירות הכללי
  const appointmentTime = new Date();
  appointmentTime.setHours(appointmentTime.getHours() + 1);
  appointmentTime.setMinutes(0, 0, 0); // עיגול לשעה עגולה
  ...
  // יצירת חבילת Starter כברירת מחדל (אם קיימת)
  ...
  if (starterPackage) {
    await prisma.subscription.create({
      data: {
        ...
        currentPeriodEnd: new Date(Date.now() + 60 * 24 * 60 * 60 * 1000), // 60 days trial
      },
    });
  }

  // יצירת תבניות התראה ברירת מחדל (WhatsApp + Email)
